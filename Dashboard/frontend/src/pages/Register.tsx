...
export default function Register() {
  ...
  const handleRegister = async() => {
    ...
    try{
      const response = await axios.post('http://localhost/See5/Dashboard/backend/register.php', {
        email,
        password,
        name,
      });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md">
        ...
        <FormInput label="نام" value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <button onClick={handleRegister} className="w-full bg-green-500 text-white py-2 rounded-lg mt-4 hover:bg-green-600">
          ثبت‌نام
        </button>
      </div>
    </div>
